...
Furthermore, PromQL users may already know that it's possible to put the metric name inside the braces, for the purpose of using an operator to enable richer metric selection, like: `{__name__="my.metric"}`. The chosen syntax serves as a shortening of this operation, and is forward-compatible with future metric matching operators if we decide to implement them.
...
* Unicode escaping: `{"emoji metric: 💩 or \u1F4A9"}`
...
Depending on the implementation, there may be UTF-8 characters that, when encountered as native values in quotes, may cause problems with the lexers and parsers. These characters could include whitespace, especially newlines, or complex combiner characters. If such characters exist, we will require that they be escaped with `\u` rather than allow them to exist inline. As development of this proposal progresses, we will discover which characters will fall into this category.
