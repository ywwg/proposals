...
2. New metrics provider, old backend: Old versions of Prometheus will throw an error if they see the new syntax. Therefore we must provide a way for the agent to serve previously-invalid metrics in a form that the old database can process.
