...
* Label names may be double-quoted.
...
For new users, it might be confusing to parse this syntax visually, with a bracket in the middle of a series of quoted values. The previous syntax resembles a function call, the way that `foo{bar}` is like `foo(bar)`. By adding the additional quotes this frame of reference is lost. It seems more intuitive to think of the brackets as **the characters that surround the parameters for selecting data**, where the "metric name" is one special case of that kind of selector. This is similar to the approach chosen for [LogQL](https://grafana.com/docs/loki/latest/query/) and [TraceQL](https://grafana.com/docs/tempo/latest/traceql/), which do not have a special case "metric name" equivalent and thus have no items outside the brackets.

### VictoriaMetrics approach

VictoriaMetrics has extended PromQL for UTF8 Support, calling the updated language [MetricsQL](https://docs.victoriametrics.com/MetricsQL.html):
